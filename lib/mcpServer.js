...
const AUTH_REQUIRED_MESSAGE =
  "This tool requires a JsonPlace API key. Public generation and one-off mock creation work without login.";
const PUBLIC_CAPABILITIES = [
  "list field options",
  "search the field catalog",
  "generate JSON payloads",
  "infer fields from JSON",
  "create public mock endpoints",
  "fetch public mock responses"
];
const ACCOUNT_CAPABILITIES = [
  ...
  "update saved mock endpoints",
  "delete saved mock endpoints"
];
...
function buildWhoamiPayload(siteUrl, user, authMode) {
  if (!user?.id) {
    return {
      authenticated: false,
      authMode: "anonymous",
      ...
      message:
        "Public JSON generation and one-off mock endpoint creation are available without login. Add an API key for saved templates and endpoint management."
    };
  }
  ...
  return {
    ...
    capabilities: [...PUBLIC_CAPABILITIES, ...ACCOUNT_CAPABILITIES],
    ...buildCurrentUserProfile(siteUrl, user)
  };
}
...
export function buildUsageGuideMarkdown(siteUrl) {
  const guide = [
    ...
    "",
    "## Public mode vs account mode",
    "- Connect without auth to generate JSON, discover fields, create one-off public mock endpoints, and read public mock responses.",
    "- Add `Authorization: Bearer <your JsonPlace API key>` when you want saved templates or mock endpoint management tied to your account.",
    "",
    ...
    "5. Add an API key only when you need saved templates or endpoint management.",
    "",
...
export function createJsonPlaceMcpServer({ db, user, authMode = "apiKey", siteUrl }) {
  ...
  server.registerTool(
    "jsonplace_whoami",
    {
      ...
      description: "Return the current JsonPlace MCP auth mode, capabilities, and user profile when available.",
      inputSchema: z.object({})
    },
    async () => {
      const profile = buildWhoamiPayload(siteUrl, user, authMode);
  ...
  server.registerTool(
    "jsonplace_list_templates",
    {
      ...
      description: "List saved templates for the authenticated JsonPlace user. Requires an API key.",
      inputSchema: z.object({})
    },
    async () => {
      const templates = await listTemplatesForUser(db, requireAuthenticatedUser(user), 100);
  ...
  server.registerTool(
    "jsonplace_save_template",
    {
      ...
      description: "Save a new template to the authenticated JsonPlace account. Requires an API key.",
      inputSchema: z.object({
        title: z.string(),
    ...
    async ({ title, description, fields }) => {
      ...
      const template = await createTemplateForUser(db, requireAuthenticatedUser(user), validation.value);
  ...
  server.registerTool(
    "jsonplace_delete_template",
    {
      ...
      description: "Delete one saved JsonPlace template. Requires an API key.",
      inputSchema: z.object({
        templateId: z.string()
      })
    },
    async ({ templateId }) => {
      await deleteTemplateForUser(db, requireAuthenticatedUser(user), templateId);
  ...
  server.registerTool(
    "jsonplace_list_mock_endpoints",
    {
      ...
      description: "List saved mock endpoints for the authenticated JsonPlace user. Requires an API key.",
      inputSchema: z.object({})
    },
    async () => {
      const endpoints = await listMockEndpointsForUser(db, requireAuthenticatedUser(user), siteUrl, 100);
  ...
  server.registerTool(
    "jsonplace_create_template_endpoint",
    {
      ...
      description: "Create a template-backed JsonPlace mock endpoint. Works anonymously or with an API key.",
      inputSchema: z.object({
        title: z.string(),
        description: z.string().optional().default(""),
  ...
  server.registerTool(
    "jsonplace_create_static_endpoint",
    {
      ...
      description: "Create a static JsonPlace mock endpoint. Works anonymously or with an API key.",
      inputSchema: z.object({
        title: z.string(),
        description: z.string().optional().default(""),
        endpointPath: z.string(),
  ...
  server.registerTool(
    "jsonplace_update_template_endpoint",
    {
      ...
      description: "Update one saved template-backed JsonPlace mock endpoint. Requires an API key.",
      inputSchema: z.object({
        endpointId: z.string(),
        title: z.string(),
        description: z.string().optional().default(""),
    ...
    async ({ endpointId, title, description, endpointPath, fields }) => {
      ...
      await trackNormalizationWarnings(db, validation.warnings);
      const endpoint = await updateMockEndpointForUser(db, requireAuthenticatedUser(user), endpointId, validation.value, siteUrl);
      return jsonResult({ endpoint }, { endpoint }, validation.warnings);
    }
  );
  ...
  server.registerTool(
    "jsonplace_update_static_endpoint",
    {
      ...
      description: "Update one saved static JsonPlace mock endpoint. Requires an API key.",
      inputSchema: z.object({
        endpointId: z.string(),
        title: z.string(),
        description: z.string().optional().default(""),
  ...
  server.registerTool(
    "jsonplace_create_mock_endpoint",
    {
      ...
      description: "Legacy polymorphic endpoint creator. Prefer jsonplace_create_template_endpoint or jsonplace_create_static_endpoint. Works anonymously or with an API key.",
      inputSchema: z.object({
        title: z.string(),
        description: z.string().optional().default(""),
        endpointPath: z.string(),
  ...
  server.registerTool(
    "jsonplace_update_mock_endpoint",
    {
      ...
      description: "Legacy polymorphic endpoint updater. Prefer jsonplace_update_template_endpoint or jsonplace_update_static_endpoint. Requires an API key.",
      inputSchema: z.object({
        endpointId: z.string(),
        title: z.string(),
        description: z.string().optional().default(""),
  ...
  server.registerTool(
    "jsonplace_delete_mock_endpoint",
    {
      ...
      description: "Delete one saved JsonPlace mock endpoint. Requires an API key.",
      inputSchema: z.object({
        endpointId: z.string()
      })
    },
    async ({ endpointId }) => {
      await deleteMockEndpointForUser(db, requireAuthenticatedUser(user), endpointId);
  ...
  server.registerTool(
    ...
    async ({ endpointPath, owner, username }) => {
      const ownerIdentifier = String(owner || username || user?.publicNamespace || user?.username || "").trim();
      if (!ownerIdentifier) {
        throw new Error("Provide owner or username when using JsonPlace MCP in anonymous mode.");
      }
