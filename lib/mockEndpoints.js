...
export function mockEndpointToClient(row, siteUrl = "", publicNamespace = "") {
  const ownerPublicNamespace = String(publicNamespace || row.ownerPublicNamespace || "").trim();
...
export async function ensureMockEndpointIndexes(db) {
  ...
  await Promise.all([
    db.collection("mock_endpoints").createIndex({ ownerUsernameLower: 1, endpointPathLower: 1 }, { unique: true }),
    db.collection("mock_endpoints").createIndex({ userId: 1, endpointPathLower: 1 }, { unique: true }),
    db.collection("mock_endpoints").createIndex({ userId: 1, createdAt: -1 })
  ]);
  indexesReady = true;
}
